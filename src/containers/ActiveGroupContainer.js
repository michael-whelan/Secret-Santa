...
const ActiveGroupContainer = () => {
    ...
    }, [user, dispatch, ugid, selectedGroup.ugid]);

    const submitFunction = () => {
        !user
            ? alert(
                  `You must be logged in and be the group admin to send emails or reactivete`
              )
    ...
    const checkEmails = () => {
        ...
        for (let i = 0; i < people.length; ++i) {
            ...
            if (!re.test(String(person.email).toLowerCase())) {
                alert(
                    `Something is wrong. Please check your email address for ${person.name}`
                );
                return false;
            }
        }
