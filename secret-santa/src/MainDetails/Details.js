...
export default class GroupDetails extends Component {
				...
				if (res.status === 202) {
					...
					if (res.data.success) {
						this.props.updateGroup(
							res.data.updated_g_id,
							res.data.updated_group[0]
						);
	...
	openDialog = (nots, event) => {
		this.setState({
			notListDialogOpen: true,
			openPerson: event.target.id,
			nots: nots,
		});
	};
	closeDialog = () => {
	...
	addPerson = sing => {
		...
		var person = (
			<div className="person" key={'person-' + single.person_id}>
				<Button
					variant="outlined"
					color="primary"
					onClick={this.showAlert.bind(
						this,
						'Are you sure you want to delete: ' + single.name
					)}
				>
					-
				</Button>
				<input
					type="text"
					key={'name-' + single.person_id}
					id={'pname-' + single.person_id}
					title={gid + '-name-' + single.person_id}
				...
				<input
					type="text"
					key={'email-' + single.person_id}
					id={'pemail-' + single.person_id}
					title={gid + '-email-' + single.person_id}
				...
				<Button
					...
					id={single.person_id}
					onClick={this.openDialog.bind(this, single.nots)}
				>
					Nots
				</Button>
			</div>
	...
	generatePeople = () => {
		...
		var dialogOptions = [];
		if (
			typeof this.state.activeGroup !== 'object' ||
			...
			<div className="Details">
				{this.props.activeGroupId !== 'null' ? (
					<>
						...
						<Button
							variant="outlined"
							color="primary"
							onClick={this.addPerson.bind(this, null)}
						>
							+
						</Button>
					</>
				...
				{(this.state.activeGroup && this.state.notListDialogOpen) ? (
					<Dialog
						...
						openDialog={this.state.notListDialogOpen}
						title={'Nots'}
						ownerId={this.state.openPerson}
						closeDialog={this.closeDialog}
						elemList={[
							{
								suggestions: this.state.dialog,
								type: 'textSuggest',
							},
						]}
						notList={this.state.nots}
